...
Write one java program that will satisfy the given requirements: 

a. It will display the given menu
b. The program will ask the user to enter his/her choice.
c. If choice is A, B, C, D, E or F, it will propt the user for the 
    required inputs, then compute and display the required outputs. 
    If choice is Q, the program will terminate. 
d. Validate all your inputs by prompting for a new value if the 
    input is invalid. 
f. Display also the descriptino of your program 
g. Program should execute for as long as the user wants to continue 
h. Use of arrays is not allowed.
*/
